...
namespace UnityStandardAssets.Characters.FirstPerson
{
    ...
    public class RigidbodyFirstPersonController : MonoBehaviour
    {
        ...
        public class MovementSettings
        {
            ...
            public float CrouchMultiplier = 0.5f;   //Speed when crouch
            ...
            [HideInInspector]
            public float delayAttackSettingsOver = 1;
...
#if !MOBILE_INPUT
                if (Input.GetButton("Run") && !isCrouched && ChokeMultiplier == 1)
	            {
        ...
        private void Update()
        {
            if (!playerKeyholePeek)
            {
                ...
                if (CrossPlatformInputManager.GetButtonDown("Jump") && !m_Jump && movementSettings.ChokeMultiplier == 1)
                {
                    m_Jump = true;
                }
                ...
                {
                    movementSettings.isCrouched = !movementSettings.isCrouched;
                    UI_Crouch.SetActive(movementSettings.isCrouched);
                }
                ...
                if (m_IsGrounded)
                {
                    ...
                    if (Physics.SphereCast(transform.position, 0.35f, Vector3.up, out hitCeil, 0.7f, layerCrouch))
                    {
                        movementSettings.isCrouched = true;
                    }
                    UI_Crouch.SetActive(movementSettings.isCrouched);
                }
            }
        ...
        private void FixedUpdate()
        {
            ...
            if (m_IsGrounded)
            {
                ...
                if (m_Jump)
                {
                    ...
                    m_RigidBody.AddForce(new Vector3(0f, movementSettings.JumpForce, 0f), ForceMode.VelocityChange);
                    m_Jumping = true;
                }
            ...
            if (movementSettings.isCrouched)
            {
                m_Capsule.height = Mathf.MoveTowards(m_Capsule.height, 1.2f, 0.7f * Time.deltaTime * 10);
                camPos.localPosition = Vector3.MoveTowards(camPos.localPosition, new Vector3(isLeaning ? (Input.GetAxis("Lean") * -0.3f) : 0, 0.5f, 0), 0.7f * Time.deltaTime * 5);
            }
        ...
        private void GroundCheck()
        {
            ...
            RaycastHit hitInfo;
            if (Physics.SphereCast(transform.position, m_Capsule.radius * (1.0f - advancedSettings.shellOffset), Vector3.down, out hitInfo,
                                   ((m_Capsule.height/2f) - m_Capsule.radius) + advancedSettings.groundCheckDistance, layerMaskGround, QueryTriggerInteraction.Ignore))
            {
                m_IsGrounded = true;
        ...
        public void IsTakingDamages()
        {
            movementSettings.isTakingDamages = true;
            CancelInvoke("TakingDamagesOver");
            Invoke("TakingDamagesOver", movementSettings.delayAttackSettingsOver);
        }


        void TakingDamagesOver()
        {
            movementSettings.isTakingDamages = false;
        }
